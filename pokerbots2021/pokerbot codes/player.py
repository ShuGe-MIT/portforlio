...
class Player(Bot):
    ...
    def __init__(self):
        ...
        self.board_strengths = [0, 0, 0] # better representation of our board strengths

        with open('hand_percentages.txt') as json_file:
            self.hole_percentages = json.load(json_file)
    ...
    def allocate_cards(self, my_cards):
        ...
        _ALLOCATE_PARAM  = 0.05
        best_cand_percentage = 0
        best_allocation = []
        for perm in itertools.permutations([0,1,2,3,4,5]):
            if perm[0]<perm[1] and perm[2]<perm[3] and perm[4]<perm[5]:
                cand_allocation = []
                cand_percentage = 0
                for i in range(6):
                    cand_allocation += [my_cards[perm[i]]]
                for i in range(3):
                    card1 = cand_allocation[2*i]
                    card2 = cand_allocation[2*i + 1]
                    if card1[1] == card2[1]:
                        is_same_suit = 's'
                    else:
                        is_same_suit = 'd'
                    hole_code = card1[0]+card2[0]+is_same_suit
                    hole_percentage = self.hole_percentages[hole_code]
                    cand_percentage += (1 + i*_ALLOCATE_PARAM) * hole_percentage
                if cand_percentage > best_cand_percentage:
                    best_cand_percentage = cand_percentage
                    best_allocation = cand_allocation


        for i in range(NUM_BOARDS): #subsequent pairs of cards should be pocket pairs if we found any
            cards = [best_allocation[2*i], best_allocation[2*i + 1]]
    ...
    def calculate_strength_board(self, hole, board, iters):
        '''
        A Monte Carlo method meant to estimate the win probability of a pair of
        hole cards. Simlulates 'iters' games and determines the win rates of our cards base on our 2 hole cards and community card presented on the board

        Arguments:
        hole: a list of our two hole cards
        board : community cards avaliable
        iters: a integer that determines how many Monte Carlo samples to take
        '''

        deck = eval7.Deck()  # eval7 object!
        hole_cards = [eval7.Card(card) for card in hole]  # card objects
        community_cards = [eval7.Card(card) for card in board]  # card objects

        for card in hole_cards:  # remove cards that we know about! they shouldn't come up in simulations
            deck.cards.remove(card)
        for card in community_cards:  # remove cards that we know is on the board
            deck.cards.remove(card)
        ...
        for _ in range(iters):  # take 'iters' samples
            ...
            opp_hole = draw [: _OPP]
            community_rest = draw [_OPP:]

            our_hand = hole_cards + community_rest + community_cards  # the two showdown hands
            opp_hand = opp_hole + community_rest + community_cards
        ...
        current_strength = score / (2 * iters)  # this is our win probability!

        return current_strength

    def has_very_strong_hand(self, hole, board):
        #detects triplets, straights, and flush
        hole_cards = [eval7.Card(card) for card in hole]  # card objects
        community_cards = [eval7.Card(card) for card in board]  # card objects
        cards = hole_cards + community_cards
        if len(cards)>=5:
            if eval7.handtype(eval7.evaluate(cards)) in ['Straight', 'Trips']:
                return True   
        return False

    def has_very_very_strong_hand(self, hole, board):
        #detects triplets, straights, and flush
        hole_cards = [eval7.Card(card) for card in hole]  # card objects
        community_cards = [eval7.Card(card) for card in board]  # card objects
        cards = hole_cards + community_cards
        if len(cards)>=5:
            if eval7.handtype(eval7.evaluate(cards)) in ['Flush', 'Full House', 'Royal Flush']:
                return True   
        return False
    ...
    def handle_new_round(self, game_state, round_state, active):
        ...
        _MONTE_CARLO_ITERS = 80 #the number of monte carlo samples we will use
        ...
        for i in range(NUM_BOARDS):
            if AssignAction in legal_actions[i]:
                ...
                if street == 0:
                    ...
                    hole = self.board_allocations[i]
                    board = board_cards[i][:street]
                    temp = self.calculate_strength_board(hole, board, _MONTE_CARLO_ITERS)
                    if street<5:
                        very_strong=0.5
                        very_very_strong=1.5
                    else:
                        very_strong=1
                        very_very_strong=2
                    if self.has_very_strong_hand(hole, board) == True:
                        temp += very_strong
                    if self.has_very_very_strong_hand(hole, board) == True:
                        temp += very_very_strong
                    self.board_strengths[i] = temp
                    strength = self.board_strengths[i]


                if street < 3: #pre-flop
                    raise_ammount = int(my_pips[i] + board_cont_cost + 0.3 * (strength) * (pot_total + board_cont_cost)) #play a little conservatively pre-flop
                elif street=3 or 4:
                    raise_ammount = int(my_pips[i] + board_cont_cost + 0.5 * (strength) * (pot_total + board_cont_cost))
                else:
                    raise_ammount = int(my_pips[i] + board_cont_cost + 0.8 * (strength) * (pot_total + board_cont_cost)) #raise the stakes deeper into the game
                ...
                if RaiseAction in legal_actions[i] and (raise_cost <= my_stack - net_cost) and random.random() < strength: #raise if we can and if we can afford it
                ...
                if board_cont_cost > 0: #our opp raised!!! we must respond
                    ...
                    if board_cont_cost > opp_stack/20:  # <--- parameters to tweak.
                        _INTIMIDATION = 0.20
                        if street >= 3:
                            if board_cont_cost > 30:
                                _INTIMIDATION += 0.15
                            if board_cont_cost > 60:
                                _INTIMIDATION += 0.10
